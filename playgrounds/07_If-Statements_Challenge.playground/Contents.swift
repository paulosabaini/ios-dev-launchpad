...
// because one equal sign is used for attribution and two equal signs is used for comparison
...
func coldWarmOrHot(guess: Int) -> String {
    ...
    return message + "\nThe number is \(randomNumber) and you guessed \(guess)."
}
...
func countToOneHundred(count: Int) -> Int {
    if count == 100 {
        ...
        return countToOneHundred(count: count + 1)
    }
}
